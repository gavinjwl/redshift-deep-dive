...
from(
    ...
    from (
        ...
        from (
            ...
            group by ss_item_sk
            having avg(ss_net_profit) > 0.90*(
                select avg(ss_net_profit) rank_col from store_sales where ss_store_sk = 639 and ss_hdemo_sk is null group by ss_store_sk
            )
        )V1
    )V11
    where rnk  < 11
